...
args.gamma=0.99
args.capacity=10000
args.num_iteration=100000
args.batch_size=64
args.seed=False
args.random_seed=9527
args.sample_frequency=256
args.render=True
args.log_interval=50
args.load=False
args.render_interval=100
...
class DDPG(object):
  def __init__(self, state_dim, action_dim, max_action):
    self.actor = actor_net(state_dim, action_dim, max_action).to(device)
    self.actor_target = actor_net(state_dim, action_dim, max_action).to(device)
    self.actor_target.load_state_dict(self.actor.state_dict())
    self.actor_optimizer = optim.Adam(self.actor.parameters(), args.learning_rate)

    self.critic = critic_net(state_dim, action_dim).to(device)
    self.critic_target = critic_net(state_dim, action_dim).to(device)
